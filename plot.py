...
def main(): 
    ...
    if args.paper_plots:
        ...
        img_models_vary = ["slow_fusion", "early_fusion", "vanilla_cnn"]
        ...
        for i, img_model in enumerate(img_models_vary):
            ...
            plot_mse(data, seq_model_fixed, img_model, "g_train_losses", show=False, save=False)
        plt.show()

        img_model_fixed = "early_fusion"
        seq_model_vary = ["lstm", "vanilla_rnn"]

        for i, seq_model in enumerate(seq_model_vary):
            model_name = f"SEQ_{seq_model}_IMG_{img_model_fixed}"
            data = pickle.load(open(f"./trained_models/{model_name}_g_train_losses.p", "rb"))
            plt.subplot(1, len(seq_model_vary), i+1)
            plot_mse(data, seq_model, img_model_fixed, "g_train_losses", show=False, save=False)
        plt.show()
